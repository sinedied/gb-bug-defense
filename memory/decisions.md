...
### Iter-4 #30: Speed-up HUD indicator via `hud_set_fast_mode()` setter
- **Date**: 2026-06-12
- **Context**: Feature #30 speed-up toggle needs to show ">>" at HUD cols 18–19 row 0 when active, replacing the tower-type letter. The fast-mode flag (`s_fast_mode`) is file-scope in game.c and must not be exposed.
- **Decision**: New `hud_set_fast_mode(bool on)` function in hud.h/hud.c stores a `static bool s_hud_fast` in hud.c. When called, it no-ops if value unchanged, otherwise sets the flag and marks `s_dirty_t = 1`. The `s_dirty_t` handler in `hud_update()` now owns BOTH cols 18 and 19: writes ">>" when fast, else " " + tower letter. The standalone `put_char(18, 0, ' ')` is removed from `hud_init()`'s static-label block (col 18 is now fully managed by the dirty handler). Post-feature, `s_dirty_t` writes 2 tiles instead of 1 on every fire (B-press tower cycle + SELECT toggle). Post-gate HUD worst case: 10 tiles (was 9). Total post-gate worst case: 15 (within 16 cap).
- **Rationale**: Avoids exposing `s_fast_mode` beyond game.c. HUD remains self-contained (reads no game.c state). The +1 tile per `s_dirty_t` event is acceptable — worst case 15 is still within budget.
- **Alternatives**: `game_is_fast_mode()` getter (rejected — exposes game.c internal state); piggyback on `hud_mark_t_dirty()` only (rejected — HUD cannot know which content to show without state).

### Iter-4 #30: Gate-lift + SELECT same-frame budget overflow accepted
- **Date**: 2026-06-12
- **Context**: If SELECT and A are pressed on the exact same frame during gate-active (first tower placement), the BG-write budget reaches 18 tiles (energy 3 + fast indicator 2 + gate restore 12 + tower tile 1 = 18). Pre-feature gate-lift was already at 16 (the cap).
...
### Iter-5: Title selector indicator = dedicated TILE_ARROW BG tile, brackets removed
- **Date**: 2026-06-16
- **Context**: User feedback: the title-screen `> < LABEL >` pattern (focus chevron + angle brackets around selector options) is "confusing" — three directional symbols compete for attention and the `> <` adjacent pair reads as meaningless. Text centering is also off because the visual group spans cols 3–15 asymmetrically.
- **Decision**: (1) Remove all `<` and `>` bracket characters from the selectors. (2) Replace the focus chevron with a new dedicated BG tile (`TILE_ARROW`, MAP_TILE_BASE+52) — a solid right-pointing filled triangle (5×6 px, black, vertically symmetric). (3) Arrow renders at col 5 of the focused selector row; unfocused row gets a space at col 5. (4) Selector labels render at cols 7–13 (7 chars). (5) The 9-tile group (arrow + gap + label) is floor-centered: `floor((20-9)/2) = 5`. (6) `draw_selector()` reduced from 11 to 7 BG writes. Priority chain unchanged. MAP_TILE_COUNT 52→53; 1 new tile.
- **Rationale**: A single solid-fill triangle is the standard Game Boy menu indicator (Pokémon, Zelda, Tetris) — immediately recognizable. Removing brackets eliminates the confusing `> <` pair. Col 5 placement keeps the visual group tightly centered. 1 new tile is negligible budget cost (181/256 total). The existing `>` font glyph (ASCII 62) is a thin 1px chevron that the user already rejected; a purpose-built solid tile reads far better at GB resolution.
- **Alternatives**: Reuse existing `>` font glyph at a new position (rejected — user explicitly dislikes it); sprite-based indicator (rejected — OAM unused on title screen but adds unnecessary complexity); keep brackets but only change the focus indicator (rejected — user called out `> <` specifically).

### Iter-6: Idle tower rescan cooldown floor (supersedes iter-3 #18 "keep cooldown=0")
- **Date**: 2026-06-17
- **Context**: Performance profiling revealed that towers with cooldown=0 and no targets in range run a full 14-enemy scan loop EVERY frame. With 16 towers idle and fast mode on (2× entity tick), this costs ~43,500 cycles/frame (62% of the ~70K budget). The iter-3 #18 decision "cooldown stays at 0, re-scan next frame" was written under the assumption that "the 14-iteration scan per frame is negligible cost on DMG" — this proved false with 16 towers.
- **Decision**: When a tower's targeting scan finds no valid target (TKIND_DAMAGE: `acquire_target` returns 0xFF; TKIND_STUN: `found_target == false`), set `cooldown = TOWER_IDLE_RESCAN - 1` (= 7, so actual rescan latency = `TOWER_IDLE_RESCAN` = 8 frames, accounting for the decrement-then-skip loop). This supersedes the iter-3 #18 "keep cooldown=0, retry next frame" rule for the empty-range case only. The "found target but all already stunned" (F1) case is unchanged — those towers still get full firing cooldown. `projectiles_fire` failure (pool full) is also unchanged — tower has a valid target and should retry quickly.
- **Rationale**: 8-frame rescan interval amortizes 16 idle towers into synchronized bursts of ~21,760 cycles every 4–8 frames (31% budget) with zero cost between bursts. Total worst-case frame: ~40K cycles (57% budget), leaving ample headroom. Max first-shot latency = 8 frames (133ms at 60fps); at bug speed 0.5px/frame, enemy moves 4 pixels in that window — well within any tower's range diameter (16–40 px). Imperceptible for tower-defense gameplay.
- **Alternatives**: Per-tower stagger offset `+ (i & 3)` (rejected — burst is already within budget at 31%; stagger adds complexity for marginal gain); spatial pre-filter grid (rejected — high implementation complexity, diminishing returns after rescan floor); O(1) enemy count cache to skip scans when pool empty (rejected — rescan floor handles this case adequately; between-wave bursts are cheap since all 14 alive-checks return false immediately).

### Iter-6: End-screen art — data-only change, no C source modifications
- **Date**: 2026-06-18
- **Context**: Game-over and victory screens are text-only placeholders ("`KERNEL PANIC`" / "`SYSTEM CLEAN`" flavour text). Title screen already demonstrates that tile-art composition creates strong visual identity. End screens should match that quality.
...
### Goomba Color compat: drop SWITCH_RAM(0) on single-bank-RAM cart
- **Date**: 2026-04-30
- **Context**: User reported the bundled `goomba.gba + bugdefender.gb` ROM hung on a white screen with a stuck audio tone when run on Goomba Color (a Game Boy emulator that runs on the GBA). The ROM ran perfectly on mGBA (DMG mode), PyBoy (CI), and on real DMG. Bisected by building progressively-stripped variants and bundling each with Goomba: minimal ROM → works; full game with `audio_init`/`save_init` skipped → works; just `save_init` skipped → works; cart type `0x03` declared but `save_init` skipped → works; `save_init` reduced to ENABLE_RAM+DISABLE_RAM (no SWITCH_RAM) → works; `save_init` reduced to SWITCH_RAM(0) only → hangs. Root cause: GBDK-2020 expands `SWITCH_RAM(b)` to a write to `0x4000` (the MBC1 RAM-bank-or-upper-ROM-bank register). On MBC1 in default banking mode 0 (the power-on state), that register sets the *upper ROM bank bits*, not the RAM bank — to actually use it for RAM banking you must first write 0x01 to `0x6000` (BANKING_MODE). Real DMG, mGBA and PyBoy treat the resulting upper-bits-=-0 write as a no-op (bank 0 is already mapped). Goomba Color's MBC1 emulator handles it less gracefully and re-banks ROM to a non-existent slot, crashing the GB CPU.
- **Decision**: Remove all `SWITCH_RAM(0)` calls from `src/save.c`. Keep `ENABLE_RAM`/`DISABLE_RAM` and the cart header at `0x03 / ROM 64 KB / RAM 8 KB` (`0x149 = 0x02`). Document the constraint in `src/save.h` and `src/save.c` so a future contributor doesn't re-add `SWITCH_RAM`.
- **Rationale**: The cart only ever uses a single 8 KB RAM bank, which is auto-selected at power-on. `SWITCH_RAM(0)` is a no-op on every platform that works — and a crash on Goomba — so removing it is strictly better for compatibility with zero behavioural change anywhere we ship. Standard GBDK practice for single-bank-RAM carts (cf. `gbdk-2020/examples/cross-platform/banks` and recent docs PRs #737 / #703 clarifying that SWITCH_RAM is only meaningful with >1 RAM bank).
- **Alternatives**: (A) Switch to MBC5 (`0x1B`) which doesn't share the upper-ROM-bits register with the RAM bank — verified working on Goomba but adds an MBC5 dep where MBC1 is the canonical small-cart choice; (B) Keep SWITCH_RAM but precede it by writing 0x01 to 0x6000 (banking mode 1) — fixes the semantics but adds a useless register write on a single-bank-RAM cart; (C) Conditional-compile a Goomba-specific build — splits CI artefacts and obscures the real bug. Removing the unnecessary call is the simplest, smallest, safest fix.
